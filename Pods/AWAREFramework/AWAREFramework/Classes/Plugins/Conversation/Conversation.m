...
//                                            conversation.blob_feature = [data objectForKey:self->KEY_BLOB_FEATURE];
    ...
    if (self != nil) {
        [self initKeys];
        _conversationsDelay = 5; // Changed from 3 to 15 minutes
        _conversationsOffDudy = 1;
        _conversationsLength = 10; // Set the collection length to 10 seconds
    }
    return self;
}
...
- (void)setParameters:(NSArray *)parameters{
    ...
    if (delay > 0) {
        _conversationsDelay = 15; // Changed from 3 to 15 minutes
    }
